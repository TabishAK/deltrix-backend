      ...
      if (user.length >= 1) {
        if (user[0].email === req.body.email) {
          return res.send({
            code: 300,
            message:
              "The email address already in use. please try to another email address",
          });
        }
        if (user[0].phone_number === req.body.phone_number) {
          return res.send({
            code: 300,
            message:
              "The phone number already in use. please try to another email address",
          });
        }
      } else {
...
account.post("/add-work-edu", (req, res) => {
  User.updateOne(
    { _id: req.body._id },
    { $set: { workNEducetion: req.body.workNEducetion } },
    (error, success) => {
      if (error) {
        res.send({
          message: "Update fail !",
...
account.post("/add-company", (req, res) => {
  User.updateOne(
    { _id: req.body._id },
    { $set: { companyDetails: req.body.companyDetails } },
    (error, success) => {
      if (error) {
        res.send({
          message: "Update fail !",
...
account.post("/add-commision", (req, res) => {
  User.updateOne(
    { _id: req.body._id },
    { $set: { commision: req.body.commision } },
    (error, success) => {
      if (error) {
        res.send({
          message: "Update fail !",
...
account.post("/use-ref-code", (req, res) => {
  User.find({ "my_ref_codes.ref_codes": req.body.ref_codes }, (err, result) => {
    if (err) {
      ...
      if (result[0]) {
        console.log(result);
        let codsObj = result[0].my_ref_codes.find(
          (v) => v.ref_codes == req.body.ref_codes
        );
        if (codsObj._id == req.body._id) {
          res.send({ code: 300, message: "You can't yous this code" });
          return;
